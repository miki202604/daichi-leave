...
// 9:00-17:00勤務（元山実希id=2, 元山通id=16, ナンid=15）
const SHORT_SHIFT_IDS = [2, 15, 16];
function getStartTimes(empId) {
  const isShort = SHORT_SHIFT_IDS.includes(empId);
  const hours = isShort
    ? [9,10,11,13,14,15,16]
    : [8,9,10,11,13,14,15,16];
  return hours.map(h => `${String(h).padStart(2,'0')}:00`);
}
function getMaxHours(empId, startHour, maxH) {
  const end = 17;
  const maxFromShift = startHour < 12 ? (12 - startHour) + (end - 13) : end - startHour;
  return Math.min(maxH, maxFromShift);
}
...
function EditModal({req, onSave, onCancel, maxHourly, maxRemain, isEmp, empId}) {
  const [date, setDate]         = useState(req.date||"");
  const [hours, setHours]       = useState(req.hours||1);
  const [startHour, setStartHour] = useState(req.start_hour||null);
  ...
  const startTimes = empId && SHORT_SHIFT_IDS.includes(empId)
    ? [9,10,11,13,14,15,16]
    : [8,9,10,11,13,14,15,16];

  function handleSave() {
    if(!date){setErr("日付を選択してください");return;}
    if(req.type==="hourly" && !startHour){setErr("開始時刻を選択してください");return;}
    ...
    onSave({date, hours: req.type==="daily"?days*HOURS_PER_DAY:hours, days, halfType, reason, startHour});
  }
  ...
  return (
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.35)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:100}}>
      <div style={{background:"#fff",borderRadius:10,padding:20,maxWidth:360,width:"calc(100% - 32px)",boxShadow:"0 4px 20px rgba(0,0,0,0.15)"}}>
        ...
        <input type="date" style={{...inp,marginBottom:12}} value={date} onChange={e=>setDate(e.target.value)}/>

        {req.type==="hourly"&&(
          <>
            <label style={lbl}>開始時刻</label>
            <select style={{...inp,marginBottom:8}} value={startHour||""}
              onChange={e=>{ setStartHour(Number(e.target.value)); setHours(1); }}>
            ...
            <select style={{...inp,marginBottom:12}} value={hours}
              ...
              disabled={!startHour}>
              {[1,2,3,4,5,6,7,8].filter(h=>{
                if(!startHour) return false;
                const maxEnd=17;
                const avail=startHour<12?(12-startHour)+(maxEnd-13):maxEnd-startHour;
                return h<=Math.min(maxH,avail);
              }).map(h=>(
                <option key={h} value={h}>{h}時間（〜{String(startHour<12?startHour+h===12?13:startHour+h:startHour+h).padStart(2,"0")}:00）</option>
              ))}
            </select>
          </>
        )}
...
export default function App(){
  ...
  const approveReq=async id=>{await supabase.from("requests").update({status:"approved"}).eq("id",id);loadData();};
  const rejectReq=async id=>{await supabase.from("requests").update({status:"rejected"}).eq("id",id);loadData();};
  const addReq=async r=>{await supabase.from("requests").insert([r]);loadData();};
  ...
  const deleteReq=async id=>{await supabase.from("requests").delete().eq("id",id);loadData();};
  ...
  if(screen==="admin_detail"&&adminDetail){
    ...
    return(<div style={{minHeight:"100vh",background:LIGHT,fontFamily:"'Noto Sans JP','Hiragino Sans',sans-serif",padding:"24px 16px 60px",maxWidth:520,margin:"0 auto"}}><style>{CSS}</style>
      ...
      {editReq&&<EditModal req={editReq} maxHourly={hourlyRemH+(requests.find(r=>r.id===editReq.id)?.hours||0)} maxRemain={remainH+(requests.find(r=>r.id===editReq.id)?.hours||0)} isEmp={false} empId={emp.id} onCancel={()=>setEditReq(null)} onSave={async(v)=>{await updateReq(editReq.id,{date:v.date,hours:v.hours,days:v.days,half_type:v.halfType,reason:v.reason,start_hour:v.startHour||null});setEditReq(null);}}/>}
    </div>);
  }
  ...
  if(screen==="emp_apply_hourly")return(<ER><button className="backBtn" onClick={()=>{setScreen("emp_home");setFormErr("");}}>← 戻る</button><div style={{fontSize:20,fontWeight:900,marginBottom:16}}>有給申請（時間単位）</div>{hourlyRemH<=0?<Alert type="error">🚫 時間単位の取得枠（{HOURLY_MAX_DAYS}日）を使い切りました。</Alert>:<Card style={{border:`1px solid ${hourlyRemH<=8?"#f0d060":"#e0e0e0"}`}}><div style={{fontSize:12,color:GRAY,marginBottom:4}}>時間単位取得 残り枠</div><div style={{fontSize:24,fontWeight:900,color:hourlyRemH<=8?GOLD:eColor(curEmp.id)}}>{hd(hourlyRemH)}</div>{hourlyRemH<=8&&<div style={{fontSize:12,color:GOLD,marginTop:4,fontWeight:600}}>⚠️ 残り枠わずかです</div>}</Card>}<Card><label style={lbl}>取得日 *</label><input type="date" style={{...inp,marginBottom:12}} value={form.date} onChange={e=>setForm(p=>({...p,date:e.target.value}))} disabled={hourlyRemH<=0}/><label style={lbl}>開始時刻</label><select style={{...inp,marginBottom:8}} value={form.startHour||""} onChange={e=>{const v=Number(e.target.value);setForm(p=>({...p,startHour:v,hours:1}));}} disabled={hourlyRemH<=0}><option value="">-- 選択 --</option>{(SHORT_SHIFT_IDS.includes(curEmp.id)?[9,10,11,13,14,15,16]:[8,9,10,11,13,14,15,16]).map(h=>(<option key={h} value={h}>{String(h).padStart(2,"0")}:00</option>))}</select><label style={lbl}>時間数（残り枠 {hd(hourlyRemH)}）</label><select style={{...inp,marginBottom:12}} value={form.hours} onChange={e=>{const v=Number(e.target.value);setForm(p=>({...p,hours:v}));}} disabled={hourlyRemH<=0||!form.startHour}>{[1,2,3,4,5,6,7,8].filter(h=>{if(!form.startHour)return false;const s=form.startHour;const maxEnd=17;const hoursAvail=s<12?(12-s)+(maxEnd-13):maxEnd-s;return h<=Math.min(hourlyRemH,remainH,hoursAvail);}).map(h=>(<option key={h} value={h}>{h}時間（〜{String(form.startHour<12?form.startHour+h===12?13:form.startHour+h:form.startHour+h).padStart(2,"0")}:00）</option>))}</select><label style={lbl}>理由（任意）</label><textarea rows={3} id="reason-hourly" placeholder="私用、通院など..." disabled={hourlyRemH<=0}/>{formErr&&<Alert type="error">{formErr}</Alert>}<Alert type="info">申請後、管理者の承認が必要です。</Alert><button className="primaryBtn" disabled={hourlyRemH<=0||!form.startHour||!form.hours} onClick={async()=>{setFormErr("");if(!form.date){setFormErr("日付を選択してください");return;}if(!form.startHour){setFormErr("開始時刻を選択してください");return;}const reasonH=document.getElementById('reason-hourly')?.value||'';await addReq({emp_id:curEmp.id,date:form.date,hours:form.hours,start_hour:form.startHour,reason:reasonH,status:"pending",type:"hourly"});setForm({date:"",hours:1,startHour:null,reason:"",halfType:"am",days:1});setScreen("emp_history");}}>申請する</button></Card></ER>);

  if(screen==="emp_apply_half")return(<ER><button className="backBtn" onClick={()=>{setScreen("emp_home");setFormErr("");}}>← 戻る</button><div style={{fontSize:20,fontWeight:900,marginBottom:16}}>有給申請（半日）</div><Alert type="success">🌓 半日取得（4時間 / 0.5日）— 時間単位枠とは別に残日数から消化されます</Alert><Card style={{marginBottom:14,display:"flex",justifyContent:"space-between",alignItems:"center"}}><span style={{fontSize:13,color:GRAY}}>現在の残り有給</span><span style={{fontSize:18,fontWeight:900,color:eColor(curEmp.id)}}>{hd(remainH)}</span></Card><Card><label style={lbl}>取得日 *</label><input type="date" style={{...inp,marginBottom:12}} value={form.date} onChange={e=>setForm(p=>({...p,date:e.target.value}))}/><label style={lbl}>午前 / 午後</label><select style={{...inp,marginBottom:12}} value={form.halfType||"am"} onChange={e=>{const v=e.target.value;setForm(p=>({...p,halfType:v}))}}><option value="am">午前半休（4時間）</option><option value="pm">午後半休（4時間）</option></select><label style={lbl}>理由（任意）</label><textarea rows={3} id="reason-half" placeholder="私用、通院など..."/>{formErr&&<Alert type="error">{formErr}</Alert>}<Alert type="info">申請後、管理者の承認が必要です。</Alert><button className="primaryBtn" disabled={remainH<4} onClick={async()=>{setFormErr("");if(!form.date){setFormErr("日付を選択してください");return;}if(remainH<4){setFormErr("残り有給が不足しています");return;}const reasonHalf=document.getElementById('reason-half')?.value||'';await addReq({emp_id:curEmp.id,date:form.date,hours:4,half_type:form.halfType||"am",reason:reasonHalf,status:"pending",type:"half"});setForm({date:"",hours:4,reason:"",halfType:"am",days:1});setScreen("emp_history");}}>申請する</button></Card></ER>);
