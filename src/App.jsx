...
function getTimeSlots(empId, maxH) {
  const isShort = SHORT_SHIFT_IDS.includes(empId);
  const start = isShort ? 9 : 8;
  const end = 17;
  const slots = [];
  for(let s = start; s < end; s++) {
    if(s === 12) continue; // 昼休み
    for(let h = 1; h <= Math.min(maxH, end - s - (s < 12 && s + h > 12 ? 1 : 0)); h++) {
      const e = s + h + (s < 12 && s + h > 12 ? 1 : 0);
      if(e > end) break;
      const label = `${String(s).padStart(2,'0')}:00 〜 ${String(e).padStart(2,'0')}:00（${h}時間）`;
      slots.push({label, hours: h, startHour: s});
    }
  }
  // 重複除去・maxH以内
  return slots.filter(s => s.hours <= maxH);
}
...
export default function App(){
  ...
  if(screen==="emp_apply_hourly")return(<ER><button className="backBtn" onClick={()=>{setScreen("emp_home");setFormErr("");}}>← 戻る</button><div style={{fontSize:20,fontWeight:900,marginBottom:16}}>有給申請（時間単位）</div>{hourlyRemH<=0?<Alert type="error">🚫 時間単位の取得枠（{HOURLY_MAX_DAYS}日）を使い切りました。</Alert>:<Card style={{border:`1px solid ${hourlyRemH<=8?"#f0d060":"#e0e0e0"}`}}><div style={{fontSize:12,color:GRAY,marginBottom:4}}>時間単位取得 残り枠</div><div style={{fontSize:24,fontWeight:900,color:hourlyRemH<=8?GOLD:eColor(curEmp.id)}}>{hd(hourlyRemH)}</div>{hourlyRemH<=8&&<div style={{fontSize:12,color:GOLD,marginTop:4,fontWeight:600}}>⚠️ 残り枠わずかです</div>}</Card>}<Card><label style={lbl}>取得日 *</label><input type="date" style={{...inp,marginBottom:12}} value={form.date} onChange={e=>{const v=e.target.value;setForm(p=>({...p,date:v}))}} disabled={hourlyRemH<=0}/><label style={lbl}>取得時間帯（残り枠 {hd(hourlyRemH)}）</label><select style={{...inp,marginBottom:12}} value={form.timeSlot||""} onChange={e=>{const v=e.target.value;const slot=getTimeSlots(curEmp.id,Math.min(hourlyRemH,remainH)).find(s=>s.label===v);setForm(p=>({...p,timeSlot:v,hours:slot?slot.hours:1,startHour:slot?slot.startHour:null}));}} disabled={hourlyRemH<=0}><option value="">-- 時間帯を選択 --</option>{getTimeSlots(curEmp.id,Math.min(hourlyRemH,remainH)).map(s=>(<option key={s.label} value={s.label}>{s.label}</option>))}</select><label style={lbl}>理由（任意）</label><textarea rows={3} id="reason-hourly" placeholder="私用、通院など..." disabled={hourlyRemH<=0}/>{formErr&&<Alert type="error">{formErr}</Alert>}<Alert type="info">申請後、管理者の承認が必要です。</Alert><button className="primaryBtn" disabled={hourlyRemH<=0||Math.min(hourlyRemH,remainH)<=0||!form.timeSlot} onClick={async()=>{setFormErr("");if(!form.date){setFormErr("日付を選択してください");return;}if(!form.timeSlot){setFormErr("時間帯を選択してください");return;}const reasonH=document.getElementById('reason-hourly')?.value||'';await addReq({emp_id:curEmp.id,date:form.date,hours:form.hours,start_hour:form.startHour,time_slot:form.timeSlot,reason:reasonH,status:"pending",type:"hourly"});setForm({date:"",hours:1,timeSlot:"",startHour:null,reason:"",halfType:"am",days:1});setScreen("emp_history");}}>申請する</button></Card></ER>);
